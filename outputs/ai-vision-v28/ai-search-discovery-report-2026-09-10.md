...
The notifier now derives URLs from docs/sitemap.xml, checks the matching deployed commit after the successful pages build and deployment workflow, rejects a superseded main branch, verifies changed public files plus the homepage, sitemap, and key, and writes a receipt containing the deployment commit, website-content hash, URL list, live checks, and response. Automated tests cover unchanged-content skips, scoped URLs, stale deployments, required assets, timeout-safe live checks, and non-success responses. Test runs never submit to Bing.
